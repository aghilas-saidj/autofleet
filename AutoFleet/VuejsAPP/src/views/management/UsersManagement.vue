        ...
        <div class="custom-container" style="min-height: 100vh">
          <div class="card">
            <div class="card-content">
              ...
              <p class="tag subtitle is-7 has-background-white">Number of users</p>
              <span class="tag is-link is-light subtitle is-7">{{ usersNumber }}</span>
            </div>
          </div>
                      ...
                      <tbody>
                        <tr v-for="user in PaginatedUsers" :key="user.id">
                          ...
                          <td>
                            <router-link
                              ...
                              class="button is-small has-background-link has-text-white mr-1"
                        ...
                        <div class="is-flex">
                          ...
                          <button
                            ...
                            @click="nextPage"
                            :disabled="sliceend >= users.length"
                          >
                            Next
                          </button>
                        </div>
...
export default {
  ...
  setup() {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`${config.API_BASE_URL}/management/users`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`, // Include token if needed
          },
        });
        users.value = response.data;
        usersNumber.value = users.value.length; // Count elements
      } catch (error) {
    ...
    const PaginatedUsers = computed(() => {
      return users.value.slice(slicestart.value, sliceend.value);
    });
